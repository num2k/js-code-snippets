...
이 사이트는 PWA 기능을 지원하여 모바일 및 데스크탑에서 앱처럼 설치하여 사용할 수 있습니다.

- **설치 방법**:
  - **데스크탑(Chrome/Edge)**: 주소창 우측의 '설치' 아이콘 클릭
  - **모바일(Chrome/Safari)**: '홈 화면에 추가' 메뉴 선택
- **주요 기능**:
  - 오프라인 상태에서도 이전에 방문한 문서 열람 가능 (캐싱)
  - 앱 아이콘을 통한 빠른 실행
  - 브라우저 UI 없이 독립된 앱 창으로 실행

플러그인: `@docusaurus/plugin-pwa`
